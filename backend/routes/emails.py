...
async def list_emails(current_user: CurrentUser, limit: int = Query(default=20, ge=1, le=100)) -> EmailListResponse:
...
async def sync_emails(
    ...
    try:
        return await email_sync_service.sync(account_email=current_user["email"], limit=limit, mode=mode)
